// <!--GAMFC-->version base on commit 43fad05dcdae3b723c53c226f8181fc5bd47223e, time is 2025-06-22 15:20:02 UTC<!--GAMFC-END-->.
// @ts-ignore
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                switch (url.pathname) {
                    ...
                    default:
                        ...
                        url.hostname = 'clubgratis.biz.id';
                        url.protocol = 'https:';
...
function getVLESSConfig(userID, hostName) {
    const vlessLink = `vless://${userID}@${hostName}:80?encryption=none&security=none&fp=randomized&type=ws&host=${hostName}&path=%2Fhttps%3A%2F%2Ft.me%2Fclub_gratis#${hostName}`
    const vlessTlsLink = `vless://${userID}@${hostName}:443?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=%2Fhttps%3A%2F%2Ft.me%2Fclub_gratis#${hostName}`
    return `

Below is TLS Port node information and sharing link，Use CF supported TLS ports：

address：${hostName} or CF preferred IP
port：443 or CF supported TLS port
UUID：${userID}
transmission：ws
...
The following is non TLS Port node information and sharing link，Available CF Supported non TLS port：
...
hint：For example, using the workers.dev domain name，The TLS port cannot be used
---------------------------------------------------------------
`;
}
